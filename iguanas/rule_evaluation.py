...
def _rule_to_expr(rule: str) -> pl.Expr:
    ...
    The first call for a given rule string calls :func:`eval` to produce an
...
def apply_rules(X: pl.DataFrame, rules: list[str]) -> pl.DataFrame:
    ...
    .. warning::
       ...
       **Rule strings are executed as Python code.** Each rule is compiled with
       :func:`eval`, so a rule string can run arbitrary Python with the
       privileges of the calling process. Only pass rules from a trusted
       source — rules generated by Iguanas itself, loaded from a trusted
       store, or reviewed by a human. **Never** pass a rule string that
       originates from untrusted or user-supplied input, and never build one
       by interpolating untrusted values. See the Notes section for the
       non-executing deployment alternative.
    ...
    Because compilation uses ``eval()``, this function must only ever be given
    trusted rule strings (see the warning above). If rules must be scored in an
    environment where their provenance cannot be guaranteed, export them with
    :func:`~iguanas.onnx_converter.rules_to_onnx` instead: that path parses the
    rule with :mod:`ast` and emits a static ONNX graph, so no Python is
    executed at scoring time.

    See Also
    --------
    apply_rules_lazy : LazyFrame equivalent (same ``eval()`` caveat).
    iguanas.onnx_converter.rules_to_onnx : Non-executing export path
...
def apply_rules_lazy(lf: pl.LazyFrame, rules: list[str]) -> pl.LazyFrame:
    ...
    .. warning::
       ...
       **Rule strings are executed as Python code.** Rules are compiled with
       :func:`eval` exactly as in :func:`apply_rules`, so a rule string can
       run arbitrary Python with the privileges of the calling process. Only
       pass rules from a trusted source — generated by Iguanas, loaded from a
       trusted store, or human-reviewed — never rules derived from untrusted
       or user-supplied input. Laziness does not mitigate this: the ``eval()``
       happens when the expression is built, before ``.collect()`` is called.
    ...
    iguanas.onnx_converter.rules_to_onnx : Export rules to a static ONNX graph
        that scores without executing Python — the recommended path when rule
        provenance cannot be guaranteed at scoring time.
    """
    if not rules:
        return lf.select([])
    exprs = [_rule_to_expr(rule) for rule in rules]
...
def apply_filter_and_deduplicate_rules(
    ...
    - Correlation filtering reduces redundancy by dropping rules whose
      predictions correlate above ``max_corr`` with a higher-ranked rule
